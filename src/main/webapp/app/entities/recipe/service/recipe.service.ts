...
export class RecipeService {
  ...
  protected convertDateFromClient(recipe: IRecipe): IRecipe {
    return Object.assign({}, recipe, {
      createdOn: recipe.createdOn?.isValid() ? recipe.createdOn.toJSON() : undefined,
      updatedOn: recipe.updatedOn?.isValid() ? recipe.updatedOn.toJSON() : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((recipe: IRecipe) => {
        recipe.createdOn = recipe.createdOn ? dayjs(recipe.createdOn) : undefined;
        recipe.updatedOn = recipe.updatedOn ? dayjs(recipe.updatedOn) : undefined;
      });
    }
    return res;
  }
