...
import { DosingUpdateComponent } from './dosing-update.component';

describe('Dosing Management Update Component', () => {
  let comp: DosingUpdateComponent;
  let fixture: ComponentFixture<DosingUpdateComponent>;
  let activatedRoute: ActivatedRoute;
  let dosingService: DosingService;
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      ...
      declarations: [DosingUpdateComponent],
      providers: [
        FormBuilder,
        {
      ...
      .overrideTemplate(DosingUpdateComponent, '')
      .compileComponents();

    fixture = TestBed.createComponent(DosingUpdateComponent);
    activatedRoute = TestBed.inject(ActivatedRoute);
    dosingService = TestBed.inject(DosingService);
  ...
  describe('ngOnInit', () => {
    it('Should call Device query and add missing value', () => {
      const dosing: IDosing = { id: 456 };
      const deviceID: IDevice = { id: 38161 };
      dosing.deviceID = deviceID;

      const deviceCollection: IDevice[] = [{ id: 20959 }];
    ...
    it('Should call PlantFactory query and add missing value', () => {
      const dosing: IDosing = { id: 456 };
      const plantFactoryID: IPlantFactory = { id: 65497 };
      dosing.plantFactoryID = plantFactoryID;

      const plantFactoryCollection: IPlantFactory[] = [{ id: 46739 }];
    ...
    it('Should update editForm', () => {
      const dosing: IDosing = { id: 456 };
      const deviceID: IDevice = { id: 88642 };
      dosing.deviceID = deviceID;
      const plantFactoryID: IPlantFactory = { id: 97715 };
      dosing.plantFactoryID = plantFactoryID;

      activatedRoute.data = of({ dosing });
      comp.ngOnInit();

      expect(comp.editForm.value).toEqual(expect.objectContaining(dosing));
  ...
  describe('save', () => {
    it('Should call update service on save for existing entity', () => {
      // GIVEN
      const saveSubject = new Subject<HttpResponse<Dosing>>();
      const dosing = { id: 123 };
      jest.spyOn(dosingService, 'update').mockReturnValue(saveSubject);
      ...
      saveSubject.next(new HttpResponse({ body: dosing }));
      saveSubject.complete();

      // THEN
      expect(comp.previousState).toHaveBeenCalled();
      expect(dosingService.update).toHaveBeenCalledWith(dosing);
    ...
    it('Should call create service on save for new entity', () => {
      // GIVEN
      const saveSubject = new Subject<HttpResponse<Dosing>>();
      const dosing = new Dosing();
      jest.spyOn(dosingService, 'create').mockReturnValue(saveSubject);
      jest.spyOn(comp, 'previousState');
      activatedRoute.data = of({ dosing });
      comp.ngOnInit();
      ...
      // THEN
      expect(dosingService.create).toHaveBeenCalledWith(dosing);
      expect(comp.isSaving).toEqual(false);
      expect(comp.previousState).toHaveBeenCalled();
    });
