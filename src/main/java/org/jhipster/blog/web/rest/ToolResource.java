...
/**
 * REST controller for managing {@link org.jhipster.blog.domain.Tool}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class ToolResource {
    ...
    public ResponseEntity<Tool> partialUpdateTool(@PathVariable(value = "id", required = false) final Long id, @RequestBody Tool tool)
        ...
        Optional<Tool> result = toolRepository
            .findById(tool.getId())
            .map(existingTool -> {
                if (tool.getgUID() != null) {
                    existingTool.setgUID(tool.getgUID());
                }
                ...
                if (tool.getManufacturer() != null) {
                    existingTool.setManufacturer(tool.getManufacturer());
                }
                if (tool.getCreatedBy() != null) {
                    existingTool.setCreatedBy(tool.getCreatedBy());
                }
                if (tool.getCreatedOn() != null) {
                    existingTool.setCreatedOn(tool.getCreatedOn());
                }
                if (tool.getUpdatedBy() != null) {
                    existingTool.setUpdatedBy(tool.getUpdatedBy());
                }
                if (tool.getUpdatedOn() != null) {
                    existingTool.setUpdatedOn(tool.getUpdatedOn());
                }
